...
class _State extends State<NcGeneratorScreen> {
  final _tool = TextEditingController(text: '1'), _diameter = TextEditingController(text: '97'), _thickness = TextEditingController(text: '10'), _passes = TextEditingController(text: '5'), _cutPerPass = TextEditingController(text: '0.5'), _cutZ = TextEditingController(text: '-13'), _x = TextEditingController(text: '0'), _y = TextEditingController(text: '0'), _plunge = TextEditingController(text: '3000'), _feed = TextEditingController(text: '1000'), _program = TextEditingController(text: 'O0001');
  late final List<TextEditingController> _controllers = [_tool, _diameter, _thickness, _passes, _cutPerPass, _cutZ, _x, _y, _plunge, _feed, _program];
  ...
  void _generate() { if (_document == null) return; try { final totalPasses = _i(_passes, 'Total passes'); if (totalPasses < 2) throw const FormatException('Total passes must be 2 or more.'); final output = NcGenerator.generate(_document!, NcParameters(drawingName: _fileName!, toolNumber: _i(_tool, 'Tool number'), toolDiameter: _d(_diameter, 'Tool diameter'), thickness: _d(_thickness, 'Thickness'), totalPasses: totalPasses, offsetDistance: _d(_cutPerPass, 'Cut per pass'), cutDepth: _d(_cutZ, 'Cutting Z').abs(), workOffset: 'G58', xOffset: _d(_x, 'X offset'), yOffset: _d(_y, 'Y offset'), plungeFeed: _i(_plunge, 'Plunge feed'), feedRough: _i(_feed, 'Rough feed'), programNumber: _program.text.trim().toUpperCase())); if (mounted) setState(() { _output = output; _error = null; }); } on Object catch (e) { if (mounted) setState(() { _output = ''; _error = e.toString().replaceFirst('Invalid argument(s): ', '').replaceFirst('FormatException: ', ''); }); } }
  ...
  Widget _setup() => AppCard(title: 'Perimeter grinding settings (G58)', child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [_dropBox(), const SizedBox(height: 16), Row(children: [Expanded(child: _field(_tool, 'Tool number')), const SizedBox(width: 12), Expanded(child: _field(_diameter, 'Tool diameter (D) — full diameter', suffix: 'mm'))]), const SizedBox(height: 12), Row(children: [Expanded(child: _field(_thickness, 'Glass thickness', suffix: 'mm')), const SizedBox(width: 12), Expanded(child: _field(_passes, 'Total passes'))]), const SizedBox(height: 12), Row(children: [Expanded(child: _field(_cutPerPass, 'Cut per pass', suffix: 'mm')), const SizedBox(width: 12), Expanded(child: _field(_cutZ, 'Cutting Z', suffix: 'mm'))]), const SizedBox(height: 12), _field(_program, 'Program number', numeric: false), const SizedBox(height: 12), Row(children: [Expanded(child: _field(_x, 'X offset', suffix: 'mm')), const SizedBox(width: 12), Expanded(child: _field(_y, 'Y offset', suffix: 'mm'))]), const SizedBox(height: 12), Row(children: [Expanded(child: _field(_plunge, 'Plunge feed', suffix: 'mm/min')), const SizedBox(width: 12), Expanded(child: _field(_feed, 'Rough feed (passes 1–2)', suffix: 'mm/min'))]), const SizedBox(height: 16), FilledButton.icon(onPressed: _document == null ? null : _generate, icon: const Icon(Icons.precision_manufacturing), label: const Text('Generate SKG1625 NC')), if (_error != null) Padding(padding: const EdgeInsets.only(top: 12), child: Text(_error!, style: const TextStyle(color: AppColors.error))), const SizedBox(height: 14), const Text('G58 is fixed. Z defaults to -13. Passes 1–2 use rough feed; remaining passes use F2000.', style: TextStyle(color: AppColors.textSecondary, fontSize: 12))]));
