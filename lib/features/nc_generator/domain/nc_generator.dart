...
class NcParameters {
  ...
  void validate(){if(toolNumber<=0||toolDiameter<=0||thickness<=0||cutDepth<=0||roughPasses<0||offsetDistance<=0||finishAllowance<0||safeZ<=rapidZ||rapidZ<=0||leadInLength<0||leadOutLength<0||zOscillation<0)throw ArgumentError('Invalid toolpath dimensions or pass configuration.');if(feedRough<=0||feedFinish<=0||plungeFeed<=0||spindleSpeed<=0)throw ArgumentError('Feeds and spindle speed must be positive.');if(!RegExp(r'^G5[4-9]$').hasMatch(workOffset))throw ArgumentError('Work offset must be G54–G59.');if(!RegExp(r'^O\d{1,8}$').hasMatch(programNumber))throw ArgumentError('Program number must use the format O0001.');}
}
